...
export default class ImageResize {
	constructor(quill, options = {}) {
		...
		// merge doesn't do arrays as you'd expect, so we'll need to apply the classes array from options separately
	...
	checkImage = evt => {
		if (this.img) {
			if (evt.keyCode == 46 || evt.keyCode == 8) {
				(Quill || Quill).find(this.img).deleteAt(0);
			}
			this.hide();
		}
	};
